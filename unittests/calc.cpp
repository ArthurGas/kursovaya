#include "calc.h"
void Calculation::operator()(int sock, Error& er)
{
    work_sock = sock;
    Calculation::calc(er);
}
...
float Calculation::multip(std::vector<float> vec_mean)
{
    float result=1.0;
...
void Calculation::calc(Error& er)
{
    int rc;
    float value;
    ...
    for (uint32_t i=0; i < count; i++) {
        ...
        float endres = overflow(multip(vec_mean));
        std::clog << "]" << std::endl;
